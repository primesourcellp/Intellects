...
import { FaPenFancy, FaChartLine, FaBullhorn, FaRegLightbulb, FaSyncAlt, FaPlus, FaMinus } from "react-icons/fa";
...
const FAQItem = ({ faq, index, sectionVariant }) => {
  ...
  return (
    <motion.div
      key={index}
      className="rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer overflow-hidden bg-white border border-yellow-200"
    ...
    >
      <div 
        className="p-6 flex justify-between items-center bg-yellow-50 hover:bg-yellow-100 transition-colors"
      ...
      >
        ...
        >
          {isOpen ? <FaMinus className="text-yellow-600" /> : <FaPlus className="text-yellow-600" />}
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <p className="text-slate-700 border-t border-yellow-200 pt-3">{faq.a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
